...
create_alpha_plot <- function(min_elevation, max_elevation, area, title) {
  ...
  plot <- ggplot(data = subset, aes(x = year, y = !!sym(area))) + 
    ...
    ggtitle(title) +
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_text(data = small_letters, aes(y = min(subset[,area], na.rm = T) - 0.05*(max(subset[,area], na.rm = T) - min(subset[,area], na.rm = T)), label = letters),
        size = 3, 
        vjust = 0,
        hjust = 0,
        nudge_x = 0)
...
create_within_plot_plot <- function(min_elevation, max_elevation, title) {
  ...
  p <- ggplot(data = subset, aes(x = year, y = beta, group = year)) +
    ...
    ggtitle(title) + 
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_text(data = small_letters, aes(x = year, y = min(subset$beta) - 0.2*(max(subset$beta) - min(subset$beta)), label = letters),
          size = 3, 
          vjust = 0,
          hjust = 0)
