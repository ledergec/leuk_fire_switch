...
compute_p_values <- function(dist_method = "bray", min_altitude = elevations[1], max_altitude = elevations[4] + 1) {
  ...
  for(i in 1:(length(years) - 1)) {
    for(j in (i+1):length(years)) {
      subset <- transformed_data[transformed_data$year %in% years[i:j] & transformed_data$elevation >= min_altitude & transformed_data$elevation < max_altitude,]
      dist_matrix_subset <- vegdist(subset[,seq(4,length(colnames(transformed_data)),1)], method = dist_method)
...
create_distance_plot <- function(dist_method, ylabel, title, p_values, min_altitude = elevations[1], max_altitude = elevations[4] + 1) {
  ...
  dist_matrix <- vegdist(subset[, seq(4,length(colnames(transformed_data)),1)], method = dist_method)
  
  box_plot_dist <- data.frame(year = character(), dist = numeric())
  years <- c("2004","2005","2006","2007","2013", "2023", "climax")
  for(year in years) {
    dist_m <- vegdist(subset[subset$year == year, seq(4,length(colnames(transformed_data)),1)], method = dist_method)
...
dist_plot <- ggarrange(bray_curtis_plot_low_el, 
                       bray_curtis_plot_mid_el, 
                       bray_curtis_plot_high_el, ncol = 1, nrow = 3)
...
create_alpha_plot <- function(min_elevation, max_elevation) {
  subset <- temp[temp$elevation >= min_elevation & temp$elevation < max_elevation,  ]
...
alpha_diversity_plot <- create_alpha_plot(elevations[1], elevations[4] + 1)

p_values_alpha_low <- compute_p_values(elevations[1], elevations[2], "low")
alpha_diversity_low_plot <- create_alpha_plot(elevations[1], elevations[2])
p_values_alpha_mid <- compute_p_values(elevations[2], elevations[3] + 1, "mid")
alpha_diversity_mid_plot <- create_alpha_plot(elevations[2], elevations[3])
p_values_alpha_high <- compute_p_values(elevations[3], elevations[4] + 1, "high")
alpha_diversity_high_plot <- create_alpha_plot(elevations[3], elevations[4] + 1)
```

Within plot Beta diversity:

```{r}
aggregated <- aggregated[aggregated$year != 2007,]
...
create_within_plot_plot <- function(min_elevation, max_elevation) {
  ...
  p <- ggplot(data = subset, aes(x = year, y = beta, group = year)) +
    ...
    xlab("Year") +
    ylab("beta diversity") + 
    ggtitle("Within Plot Beta-Diversity") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_text(data = small_letters, aes(x = year, y = quant, label = letters),
          size = 3, 
          vjust = -0.5,
...
within_betadiversity_by_year_plot <- create_within_plot_plot(elevations[1], elevations[4] + 1)
```
