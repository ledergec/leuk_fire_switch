...
raw_data$year[raw_data$year == "climax"] <- 2178 # treat climax data as being 200 years after the fire
...
mosses <- read_csv("../Daten/Intermediate/moss_taxa.csv")
raw_data <- raw_data[!(raw_data$`Species_unified_2004-23` %in% mosses$moss_taxa),]
...
colnames(summary)
correlation_expect_with_midolo <- 
  cor(summary[!is.na(summary$Disturbance.Severity),
              c("iv", "Disturbance.Frequency", "Disturbance.Severity", "Disturbance.Frequency.herblayer",
                "Disturbance.Severity.herblayer", "Mowing.Frequency", "Grazing.Pressure", "Soil.Disturbance")])

write_csv(as.data.frame(correlation_expect_with_midolo), 
          "../Daten/Results/Disturbance Indicator Values/correlation_hof_expect_with_midolo.csv")

correlation_expect_with_landolt <- 
  cor(summary[!is.na(summary$Competition),c("iv", "Competition", "Stress", "Ruderalness", "EM", "MV")], use="complete.obs")

write_csv(as.data.frame(correlation_expect_with_landolt), 
          "../Daten/Results/Disturbance Indicator Values/correlation_hof_expect_with_landolt.csv")
```
...
correlation_expect_with_dry_seed_mass <- 
  cor(summary$dry_seed_mass_mg[!is.na(summary$dry_seed_mass_mg)], summary$iv[!is.na(summary$dry_seed_mass_mg)])
correlation_expect_with_dry_seed_mass
correlation_expect_with_sla <- 
  cor(summary$sla_cm2_per_mg[!is.na(summary$sla_cm2_per_mg)], summary$iv[!is.na(summary$sla_cm2_per_mg)])
correlation_expect_with_sla
correlation_expect_with_canopy_height <- 
  cor(summary$canopy_height_m[!is.na(summary$canopy_height_m)], summary$iv[!is.na(summary$canopy_height_m)])
correlation_expect_with_canopy_height

write_csv(summary, "../Daten/Intermediate/fire_disturbance_aggregated_data.csv")
```

Try to do the same with optimum:
```{r}
summary <- summary[summary$successional == "unimodal",]

correlation_expect_with_midolo <- 
  cor(summary[!is.na(summary$Disturbance.Severity),
              c("optimum", "Disturbance.Frequency", "Disturbance.Severity", "Disturbance.Frequency.herblayer",
                "Disturbance.Severity.herblayer", "Mowing.Frequency", "Grazing.Pressure", "Soil.Disturbance")])
correlation_expect_with_midolo

correlation_expect_with_landolt <- 
  cor(summary[!is.na(summary$Competition),c("optimum", "Competition", "Stress", "Ruderalness", "EM", "MV")], use="complete.obs")
correlation_expect_with_landolt

correlation_expect_with_dry_seed_mass <- 
  cor(summary$dry_seed_mass_mg[!is.na(summary$dry_seed_mass_mg)], summary$optimum[!is.na(summary$dry_seed_mass_mg)])
correlation_expect_with_dry_seed_mass
correlation_expect_with_sla <- 
  cor(summary$sla_cm2_per_mg[!is.na(summary$sla_cm2_per_mg)], summary$optimum[!is.na(summary$sla_cm2_per_mg)])
correlation_expect_with_sla
correlation_expect_with_canopy_height <- 
  cor(summary$canopy_height_m[!is.na(summary$canopy_height_m)], summary$optimum[!is.na(summary$canopy_height_m)])
correlation_expect_with_canopy_height
```
...
benchmark_results <- round(cor(aggregated_data[,c("log_year_since_fire", "mean_iv", "mean_disturbance_frequency", "mean_disturbance_severity", "mean_frequency_herblayer", "mean_severity_herblayer", "mean_mowing_frequency", "mean_grazing_pressure", "mean_soil_disturbance", "mean_competition", "mean_stress", "mean_ruderalness", "mean_hemeroby", "mean_mowing_resistance", "mean_dry_seed_mass", "mean_sla", "mean_canopy_height")]), 2)
write_csv(as.data.frame(benchmark_results), "../Daten/Results/Disturbance Indicator Values/benchmark_results.csv")

p_iv <- ggplot(data = aggregated_data, aes(x=log(aggregated_data$year-2003), y=mean_iv)) +
  geom_point() +
  geom_smooth(method = "lm") +
  xlab("log10(year since fire)") +
  ylab("mean FDIV") +
  theme_classic()

p_sd <- ggplot(data = aggregated_data, aes(x=log(aggregated_data$year-2003), y=mean_soil_disturbance)) +
  geom_point() +
  geom_smooth(method = "lm") +
  xlab("log10(year since fire)") +
  ylab("mean Soil Disturbance") +
  theme_classic()

p_sh <- ggplot(data = aggregated_data, aes(x=log(aggregated_data$year-2003), y=mean_severity_herblayer)) +
  geom_point() +
  geom_smooth(method = "lm") +
  xlab("log10(year since fire)") +
  ylab("mean Herblayer Severity") +
  theme_classic()

p_h <- ggplot(data = aggregated_data, aes(x=log(aggregated_data$year-2003), y=mean_hemeroby)) +
  geom_point() +
  geom_smooth(method = "lm") +
  xlab("log10(year since fire)") +
  ylab("mean Hemeroby") +
...
joined_plot <- ggarrange(p_iv, p_sd, p_sh, p_h)
```
...
summary$iv <- normalize(summary$iv)
...
diff <- summary$iv - summary$Soil.Disturbance
which(diff == max(diff, na.rm = T))
summary[14,]$species_name
order(diff)

diff[order(diff)]
summary[order(diff),]$species_name
summary[order(diff),]$Soil.Disturbance
summary[order(diff),]$iv
...
temp <- summary[,c("species_name", "iv", "Disturbance.Frequency", "Disturbance.Severity", "Soil.Disturbance", "Mowing.Frequency", "Grazing.Pressure", "EM", "MV")]
temp <- pivot_longer(temp, c( "iv", "Disturbance.Frequency", "Disturbance.Severity", "Soil.Disturbance", "Mowing.Frequency", "Grazing.Pressure", "EM", "MV"))

temp |>
  ggplot(aes(x = value, colour = name, na.rm = T)) + 
    geom_density() 

temp <- summary[,c("species_name", "iv", "Competition", "Stress", "Ruderalness", "dry_seed_mass_mg", "sla_cm2_per_mg", "canopy_height_m")]
temp <- pivot_longer(temp, c( "iv", "Competition", "Stress", "Ruderalness", "dry_seed_mass_mg", "sla_cm2_per_mg", "canopy_height_m"))
