...
analyze_seed_backpressure <- function(data, title) {
  length(unique(data$global_id[data$year == "goedicke"]))
  length(unique(data$global_id[data$year == "climax"]))
  
  goedicke_species <- unique(data$`Species_unified_2004-23`[data$year == "goedicke"])
  goedicke_species
  ...
  species_venn <- setVennRegion(species_venn, c('2004-2023'), as.numeric(sum(!(fire_species[!(fire_species %in% climax_species)] %in% goedicke_species))))
  species_venn <- setVennRegion(species_venn, c('Before'), as.numeric(sum(!(goedicke_species[!(goedicke_species %in% climax_species)] %in% fire_species))))
  species_venn <- setVennRegion(species_venn, c('Intact'), as.numeric(sum(!(climax_species[!(climax_species %in% fire_species)] %in% goedicke_species))))
  species_venn <- setVennRegion(species_venn, c('2004-2023', 'Before', 'Intact'), as.numeric(sum(fire_species[fire_species %in% climax_species] %in% goedicke_species)))
  species_venn <- setVennRegion(species_venn, c('2004-2023', 'Before'), as.numeric(sum(!(fire_species[fire_species %in% goedicke_species] %in% climax_species))))
  species_venn <- setVennRegion(species_venn, c('2004-2023', 'Intact'), as.numeric(sum(!(fire_species[fire_species %in% climax_species] %in% goedicke_species))))
  species_venn <- setVennRegion(species_venn, c('Before', 'Intact'),  as.numeric(sum(!(goedicke_species[goedicke_species %in% climax_species] %in% fire_species))))
